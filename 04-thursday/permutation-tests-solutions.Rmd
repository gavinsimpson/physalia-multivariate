...
## Loading the packages you'll need

Start by loading the packages you will need to complete *all* of the examples. Not all packages are needed for each example, but we load them all now, so that it is easy for you to pick and choose from the examples that you want to work with.
...
Below, a series of examples are described, with (hopefully) just enough information for you to be able to think about how to fit a model to test a stated hypothesis, and also how to design the correct permutation test using `how()`. It might help to sketch out the experimental design in rough form, and use different colours for the treatments as you colour in the blocks/plots/samples as I did in the slides; think about whether a treatment varies at the sample level or at the plot level.

If you are stuck, or just want to know how to analyse a particular example, you can look at the [solutions file](https://gavinsimpson.github.io/physalia-multivariate/04-thursday/permutation-tests-solutions.html).
...
This example uses a data set of Swedish pine forest undergrowth assemblages that were experimentally manipulated through the addition of nitrogen `N` and phosphorus `P`. There are 32 samples in 4 complete, randomized blocks of 8 plots each. In each block, all combinations of 4 levels of nitrogen `N` (applied as ammonium nitrate) and 2 levels of phosphorus `P` (applied as compound PK fertilizer) were applied.
