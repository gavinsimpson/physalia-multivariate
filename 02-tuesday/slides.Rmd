...
PCA is fitted using `pca()` given a data frame of Observations
...
A transformation-based PCA ([Legendre & Gallagher 2001](https://doi.org/10.1007/s004420100716)) is where we transform the response such that when we run PCA it preserves the non-Euclidean distance between samples

Here, with a *Hellinger* transformation, the PCA will reflect the hellinger distances in the original data

```{r vare-pca-1, echo = TRUE}
ord <- pca(decostand(varespec, method = "hellinger"), scale = TRUE)
ord
```
