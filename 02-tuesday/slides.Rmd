...
What is .text-highlight[unconstrained]?
...
```{r pca-animation-setup, cache = TRUE}
## For some reason this is not running via Make
## Create Data
set.seed(1)
...
```{r pca-animation, eval = FALSE}
# For some reason this won't run when called from make
pca_anim <- animate(
...
## compute principal component in both coord systems
...
This arrangement is back-to-front if you are used to OTU/ASV like data
...
# PCA of lichen pasture soil data

PCA is fitted using function `pca()` given a data frame of observations

We can do PCA on two versions of the data $Y$

1. PCA of the covariance matrix of $Y$, with `scale = FALSE`, is the default, or

2. PCA of the correlation matrix of $Y$, with `scale = TRUE`.

PCA of the covariance matrix requires all observations are in the same units, and magnitudes dominate

---

# PCA of lichen pasture soil data

Soil property variables for the lichen pastures data are measured in different units, so PCA of the correlation matrix of $Y$ is advisable

Scale all variables to be &mu; = 0, &sigma;<sup>2</sup> = 1 random variables with `scale = TRUE`

```{r varechem-pca, echo = TRUE}
varechem_ord <- pca(varechem, scale = TRUE)
```
...
# Vegetation in lichen pastures &mdash; tb-PCA

PCA is not generally recommended for species composition data

Transformation-based PCA ([Legendre & Gallagher 2001](https://doi.org/10.1007/s004420100716)) is a viable option

Transform the response such that when we run PCA it preserves the non-Euclidean distance between samples

---

# Vegetation in lichen pastures &mdash; tb-PCA

$$d_{ij}^{\text{Hellinger}} = \sqrt{\sum_{k = 1}^{m} \left ( \sqrt{\frac{y_{ik}}{y_{i+}}} - \sqrt{\frac{y_{jk}}{y_{j+}}}\right )^2}$$

Notice this looks like the Euclidean distance

$$d_{ij}^{\text{Euclidean}} = \sqrt{\sum_{k = 1}^{m} \left ( y_{ik} - y_{jk}\right )^2}$$

on transformed data: to percentages by division by $y_{i+}$, then square-root transform

---

# Vegetation in lichen pastures &mdash; tb-PCA

Therefore, if we transform the data first, when we run that transformed data through PCA, the distance preserved is no longer the Euclidean distance but some other distance

With Hellinger transformation, the distance preserved is the Hellinger distance
...
1. Hellinger transformation $y^{\prime}_{ij} = \sqrt{\frac{y_{ij}}{y_{i+}}}$, and

2. Chi-square distance transformation $y^{\prime}_{ij} = \sqrt{y_{++}}\frac{y_{ij}}{y_{i+}\sqrt{y_{+j}}}$

3. Chord transformation $y^{\prime}_{ij} = \frac{y_{ij}}{\sqrt{\sum_{j=1}^m y_{ij}^2}}$

With Chi-square transformation followed by PCA of $Y^{\prime}$ we get something close to CA of $Y$

Not exactly the same as CA of $Y$ because CA is a *weighted* method

---

# Vegetation in lichen pastures &mdash; PCA

A transformation-based PCA with a *Hellinger* transformation, the PCA will reflect the hellinger distances in the original data

```{r vare-pca-1, echo = TRUE}
ord <- decostand(varespec, method = "hellinger") |> # apply transform
  pca(scale = TRUE)                                 # apply PCA
ord
```
...
Visualise ordinations with a biplot or joint plot

```{r vare-pca-1c, echo = TRUE, out.width = "85%", fig.align="center", dev.args = list(pointsize = 14)}
biplot(ord, scaling = "symmetric")
```
...
"Important" axes are ones that explain more variation than null model

Null model is the broken stick distribution

Plot &lambda; for each axis on a *screeplot*

```{r vare-pca-3, echo = TRUE}
eigenvals(ord) |> head(5)
```
]
.col-5[
...
Correspondence analysis (CA) is *in princple* very similar to PCA &mdash; a weighted form of PCA
...
CA assumes a "species packing model"
...
- `scaling = -1` &mdash; As above, but for `pca()` & `rda()` get correlation scores
- `scaling = -2` &mdash; for `ca()` or `cca()` multiply results by $\sqrt{(1/(1-\lambda_i))}$
- `scaling = -3` &mdash; this is Hill's scaling
- `scaling < 0` &mdash; For `pca()`, `rda()` divide species scores by species' $\sigma$
...
For PCA (`pca()`) use `correlation = TRUE` to get the correlation scores (negative scaling)

For CA (`ca()`)  use `hill = TRUE` to get Hill's scaling scores (negative scaling)
...
head(scores(ord, choices = 1:3, display = "species", scaling = "species", correlation = TRUE))
```
...
pco1 <- pco(vegdist(varespec), eig = TRUE)
round(eigenvals(pco1), 3)
pco2 <- pco(vegdist(varespec), eig = TRUE, add = "lingoes")
...
pco3 <- pco(vegdist(varespec), eig = TRUE)

## plot
plot(pco3)

## get PCoA scores
scrs <- scores(pco3, choices = 1:2)
...
disp <- "species"
scl <- "symmetric"
plot(ord, display = disp,
     scaling = scl, type = "n")
points(ord, display = disp,
       scaling = scl, pch = 19,
       col = "#025196")
set.seed(10)
ordipointlabel(ord,
...
plot(ord, display = disp,
     scaling = scl, type = "n")
points(ord, display = disp[1],
       scaling = scl, pch = 19,
       col = "#025196")
points(ord, display = disp[2],
       scaling = scl, pch = 19,
       col = "#fdb338")
set.seed(10)
ordipointlabel(ord,
...
d_ord <- decostand(dune, method = "hellinger") |>
  pca()

plot(d_ord, type = "n", scaling = scl,
     display = disp)
cols <- with(dune.env, col_vec[Management])
points(d_ord, display = disp, scaling = scl,
...
plot(d_ord, type = "n", scaling = scl,
     display = disp)
ordihull(d_ord, groups = dune.env$Management,
         col = col_vec,
         scaling = scl, lwd = 2)
ordispider(d_ord, groups = dune.env$Management,
           col = col_vec,
...
plot(d_ord, type = "n", scaling = scl,
     display = disp)
## ellipsoid hull
ordiellipse(d_ord, groups = dune.env$Management,
...
The **ggvegan** package is in development but has had its first release to CRAN

```{r install-ggvegan, eval = FALSE, echo = TRUE}
# Install from CRAN
install.packages("ggvegan")
# Install development version binary from R-universe:
...
head(ford, 4) # returns scores as tibble
...
# ggvegan &mdash; `autoplot()`

```{r ggvegan-3, echo=TRUE, out.width="90%", fig.align="center"}
autoplot(ord, scaling = "symmetric")
```
